...
const NewProblemModal = ({ visible, closeModal }) => {
  ...
  const renderFooter = useCallback(
    props => (
      <BottomSheetFooter {...props}>
        <View className="flex-row px-4 py-2 space-x-2 bg-white" onLayout={event => {
  ...
  return (
    <View className="w-full h-full">
      {/* <View className="border-green-300 border-2 w-full">
        <Text>Awesome 🎉</Text>
      </View> */}
      <BottomSheet
        ref={ref}
        ...
        <View className="">
          ...
          <ScrollView 
            ...
            contentContainerStyle={{paddingBottom: safeAreaInsets.bottom + 56 /*extraScrollHeight*/}}
            >
      ...
      {/* <Portal>
        <Modal visible={visible} className="w-full justify-end mb-0" onDismiss={() => onCancelPress()} visible={visible}>
          <View className="flex-col bottom-0 rounded-t-xl" style={{backgroundColor: theme.colors.surface}}>
            <View className="flex-row">
              <Text className="my-6 ml-6 grow self-end" variant="headlineMedium">New problem</Text>
            </View>
              ...
              <ScrollView 
                ...
                <ScrollView showsHorizontalScrollIndicator={false} horizontal className="mb-2">
                  <View className="flex-row grow px-4 gap-x-2">
                    <TouchableOpacity onPress={() => {onOpenCamera()}} activeOpacity={0.5}>
                      <View
                        className="flex rounded-lg w-24 h-48 border-dashed border-2 border-blue-500 items-center justify-center">
                        <MaterialCommunityIcons name="camera" size={32} color="#3b82f6" className=""/>
